...
def init():
    ...
    parser.add_argument("-t", "--task", type=str, help="which dataset you would like to test on",
                        ...
                        choices=["ASVspoof2019LA", "ASVspoof2015", "VCC2020", \
                                 "ASVspoof2019LASim", "ASVspoof2021LA", "singfake", "opencpopacesinger", "svdd2024"])
...
def test_model_on_opencpopacesinger(feat_model_path, loss_model_path, dataset_path, add_loss, args):
    model = torch.load(feat_model_path)
    loss_model = torch.load(loss_model_path) if add_loss is not None else None
    test_set = OpencpopVSVisinger(partition="test", target_sr=16000) #OpencpopACESinger(partition="test")
    testDataLoader = DataLoader(test_set, batch_size=args.batch_size, shuffle=False, num_workers=0)
    model.eval()
    with torch.no_grad():
        idx_loader, score_loader = [], []
...
def test_model_on_svdd2024(feat_model_path, loss_model_path, dataset_path, add_loss, args):
    model = torch.load(feat_model_path)
    loss_model = torch.load(loss_model_path) if add_loss is not None else None
...
if __name__ == "__main__":
    ...
    elif args.task == "svdd2024":
        eer = test_model_on_svdd2024(model_path, loss_model_path, "/data2/neil/svdd2024/", args.loss, args)
    elif args.task == "opencpopacesinger":
        eer = test_model_on_opencpopacesinger(model_path, loss_model_path, "/data2/neil/opencpopacesinger/", args.loss, args)
    else:
